    ...
    loadScript()
      .then(() => {
        ...
        const pollOnce = async () => {
          try {
            const r = await fetch(`/api/metrics?keys=cpu,memory,disk,active_tasks`, { cache: "no-store" });
            if (!r.ok) throw new Error("metrics");
            const j = await r.json();
            const cpu = Number(j?.cpu ?? NaN);
            const mem = Number(j?.memory ?? NaN);
            if (!Number.isNaN(cpu) && cpuChart) push(cpuChart, Math.max(0, Math.min(100, cpu)));
            if (!Number.isNaN(mem) && memChart) push(memChart, Math.max(0, Math.min(100, mem)));
            setMeta({ disk: Number(j?.disk), active_tasks: Number(j?.active_tasks) });
            setLoading(false);
          } catch {
  ...
  return (
    <div className="grid grid-cols-12 gap-4 mt-6">
      <div className="col-span-12 sm:col-span-6 lg:col-span-3">
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md shadow-[0_8px_32px_rgba(0,0,0,0.35)] p-4">
          ...
          <div className="mt-2 h-16 relative">
            {loading && <div className="absolute inset-0 grid place-items-center text-white/50 text-xs">Loading…</div>}
            <canvas ref={cpuRef} />
          </div>
        </div>
      </div>
      ...
      <div className="col-span-12 sm:col-span-6 lg:col-span-3">
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md shadow-[0_8px_32px_rgba(0,0,0,0.35)] p-4">
          <div className="flex items-center justify-between">
            <div className="text-white/70 text-xs uppercase tracking-wide">Active Tasks</div>
            <span className="text-[#22d3ee]">#</span>
          </div>
          <div className="mt-3 text-2xl font-extrabold text-white">{Number.isFinite(meta.active_tasks as any) ? meta.active_tasks : "—"}</div>
      ...
      <div className="col-span-12 sm:col-span-6 lg:col-span-3">
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md shadow-[0_8px_32px_rgba(0,0,0,0.35)] p-4">
          <div className="flex items-center justify-between">
            <div className="text-white/70 text-xs uppercase tracking-wide">Disk</div>
            <span className="text-[#a855f7]">%</span>
          </div>
          <div className="mt-3 text-2xl font-extrabold text-white">{Number.isFinite(meta.disk as any) ? `${meta.disk}%` : "—"}</div>
