...
function supabaseAdapter(): DBAdapter {
  ...
  return {
    tasks: {
      async upsertMany(rows) { return { inserted: 0, updated: 0 } },
      async list() { return [] },
    },
    logs: { async appendMany(rows) { return { inserted: 0 } } },
    metrics: { async appendMany(rows) { return { inserted: 0 } } },
    scheduler: {
      async listJobs() { return [] },
      async saveJobs(jobs) { return { ok: false } },
      async appendHistory(rows) { return { inserted: 0 } },
      async listHistory(limit = 100) { return [] },
    },
    sessions: { async snapshot(snap) { return { ok: false } } },
    heartbeat: { async write(row) { return { ok: false } } },
  }
}
...
function githubAdapter(): DBAdapter {
  ...
  return {
    tasks: {
      async upsertMany(_rows) { return { inserted: 0, updated: 0 } },
      async list() { return [] },
    },
    logs: { async appendMany(_rows) { return { inserted: 0 } } },
    metrics: { async appendMany(_rows) { return { inserted: 0 } } },
    scheduler: {
      async listJobs() { return [] },
      async saveJobs(_jobs) { return { ok: true } },
      async appendHistory(_rows) { return { inserted: 0 } },
      async listHistory(_limit = 100) { return [] },
    },
    sessions: { async snapshot(_snap) { return { ok: true } } },
    heartbeat: { async write(_row) { return { ok: true } } },
  }
}
