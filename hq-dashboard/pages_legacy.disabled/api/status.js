  ...
  try {
    ...
    try {
      if (fs.existsSync(statusPath)) {
        ...
        const micro =
          text.match(/Micro-Commit:\s*(YES|NO)/i)?.[1]?.toUpperCase() || null;
        const skill =
          text.match(/Skill-Fortschritt:\s*(YES|NO)/i)?.[1]?.toUpperCase() ||
          null;
        const state =
          text.match(/State aktualisiert:\s*(YES|NO)/i)?.[1]?.toUpperCase() ||
          null;
        kpis = {
          microCommit: micro,
          skillProgress: skill,
