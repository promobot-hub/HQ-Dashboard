...
export default function SkillActivityModal({ id, onClose }: { id: string | null; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const r = await fetch(`/api/logs?limit=50&skillId=${encodeURIComponent(id!)}`, { cache: "no-store" });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative max-w-2xl w-[92vw] md:w-[720px] rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md shadow-[0_8px_32px_rgba(0,0,0,0.55)] p-4">
        <div className="flex items-center justify-between">
          <div className="text-white font-semibold">Activity for Skill {id}</div>
        ...
        <div className="mt-3 max-h-[60vh] overflow-auto space-y-2 pr-1">
          ...
          {!loading && items.length === 0 && <div className="text-white/60 text-sm">No activity.</div>}
          {items.map((x, i) => (
