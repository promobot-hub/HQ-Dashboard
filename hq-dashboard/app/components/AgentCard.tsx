...
function Heatmap({ activity }: { activity?: Array<string> }) {
  ...
  (activity || []).forEach((ts) => {
    const ageH = Math.floor((now - Date.parse(ts)) / (60 * 60 * 1000));
    const idx = 23 - Math.max(0, Math.min(23, ageH));
...
export default function AgentCard({
  ...
  const ok =
    ...
    (!agent.paused && agent.status !== "degraded");
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="text-white font-semibold truncate flex items-center gap-2">
            {agent.name || agent.id}
            {agent.paused && (
              <span className="rounded-md bg-amber-400/20 text-amber-200 px-1.5 py-0.5 text-[10px]">
                PAUSED
              </span>
            )}
          </div>
        ...
        <button
          ...
          onClick={() => onToggle(agent.id, !agent.paused)}
      ...
      <div className="mt-1 text-white/60 text-xs">
        ...
        {agent.lastActiveAt
          ? (() => {
              const ms = Date.now() - Date.parse(agent.lastActiveAt as string);
      ...
      <div className="mt-2">
        <Heatmap activity={agent.activity} />
      </div>
    </div>
  );
}
