...
export default function QuickStats() {
  ...
  const runsUp = useMemo(() => {
    const up =
      prevRuns.current != null &&
      (data?.runsToday ?? 0) > (prevRuns.current as number);
    prevRuns.current = data?.runsToday ?? null;
    return up;
  }, [data?.runsToday]);
  const prevTotal = useRef<number | null>(null);
  const totalTrend = useMemo(() => {
    const cur = data?.totalRuns ?? null;
  ...
  const latTrend = useMemo(() => {
    const cur = health?.proxyLatencyMs ?? null;
    const prev = prevLatency.current;
    const res =
      prev != null && cur != null
        ? cur < prev
          ? "up"
          ...
          : "flat"
        : "flat";
    prevLatency.current = cur;
    return res;
  }, [health?.proxyLatencyMs]);
    ...
    <div className="grid grid-cols-2 sm:grid-cols-6 gap-3 w-full md:w-auto">
      ...
      <Stat
        ...
        value={
          <span>
            {data?.totalRuns ?? "—"}
            <Trend up={totalTrend === "up"} down={totalTrend === "down"} />
          </span>
        }
      />
      ...
      <Stat
        ...
        value={
          <span>
            {data?.ok || data?.lastRunAt ? "OK" : "DOWN"}
            <Trend up={latTrend === "up"} down={latTrend === "down"} />
          </span>
        }
        accent={
