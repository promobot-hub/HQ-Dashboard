...
export default function RunsSparkline() {
  ...
  const points = useMemo(() => {
    ...
    const arr = (items||[]).filter(x=>x.ts && (now - Date.parse(String(x.ts))) < win);
    // group per hour
    const buckets: number[] = Array.from({length:24},()=>0);
    arr.forEach(x => { const ageH = Math.floor((now - Date.parse(String(x.ts)))/(60*60*1000)); const idx = Math.max(0, Math.min(23, 23-ageH)); buckets[idx] += (x.ok===false? -1 : 1); });
    return buckets;
  }, [items]);
  ...
  const path = points.map((v,i)=>{
    const x = i*step; const y = h/2 - (v/max)*(h/2-2);
  ...
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} className="mx-auto">
      ...
      <rect x="0" y="0" width={w} height={h} rx="6" fill="rgba(255,255,255,0.06)" />
      <path d={path} fill="none" stroke="url(#sp-grad)" strokeWidth="2" strokeLinecap="round"/>
    </svg>
  );
}
