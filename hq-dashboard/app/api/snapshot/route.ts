...
export async function POST() {
  try {
    ...
    const [tasksR, hbR, logsR] = await Promise.all([
      fetch(`${CLAWBOT_API_BASE}/api/tasks`, { cache: "no-store" }).catch(
        () => null
      ),
      fetch(`${CLAWBOT_API_BASE}/api/heartbeat`, { cache: "no-store" }).catch(
        () => null
      ),
      fetch(`${CLAWBOT_API_BASE}/api/logs?limit=50`, {
        cache: "no-store",
      }).catch(() => null),
    ]);
    ...
    if (repo && token) {
      ...
      const tcommit = await ghPutContent(
        repo,
        tasksPath,
        tasksContent,
        `chore(tasks): sync from core @ ${ts}`
      );
      results.tasksUpdated = tcommit.ok;
    }

    return NextResponse.json(
      { ok: true, tasks, heartbeat, logs },
      { status: 200 }
    );
  } catch (e: any) {
    return NextResponse.json(
