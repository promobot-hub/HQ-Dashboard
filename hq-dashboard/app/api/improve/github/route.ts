...
export async function POST(req: NextRequest) {
  try {
    ...
    const payload = body && body.trim().length ? body : JSON.stringify({ trigger: true, ts }, null, 2);
    const path = `data/improve_trigger-${ts}.json`;
    const r = await ghPutContent(repo, path, typeof payload === 'string' ? payload : JSON.stringify(payload, null, 2), `chore(improve): trigger @ ${ts}`);
