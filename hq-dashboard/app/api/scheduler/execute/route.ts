...
async function runJob(job: any) {
  const base = process.env.SELF_BASE || process.env.CRON_SELF_URL || "https://hq-dashboard-z74i.onrender.com";
  ...
  try {
    const r = await fetch(base + endpoint, { method: 'POST', headers: { 'Content-Length': '0' } });
...
export async function POST() {
  try {
    ...
    if (!repo || !token) return NextResponse.json({ ok: false, error: 'missing_github_env' }, { status: 400 });
    ...
    for (const j of due) {
      ...
      j.nextRunAt = new Date(Date.now() + Math.max(60000, Number(j.everyMinutes||5)*60000)).toISOString();
      const hist = { ts: j.lastRunAt, jobId: j.id, name: j.name, endpoint: res.endpoint, status: res.status, ok: res.ok };
      await appendHistory(repo, token, JSON.stringify(hist));
    }
    if (due.length) await saveJobs(repo, token, { jobs });
    return NextResponse.json({ ok: true, ran: results, totalJobs: jobs.length, due: due.map(d=>d.id) }, { status: 200 });
  } catch (e: any) {
