...
async function rawOk(path: string) {
  ...
  for (const base of BASES) {
    try {
      ...
      if (ok)
        return { ok: true, status: r.status, durationMs: Date.now() - t0, url };
    } catch {}
  }
  return {
    ok: false,
...
export async function GET(req: NextRequest) {
  ...
  // schedulerExecute: append a noop history line via GH if creds available
  {
    ...
    try {
      ...
      if (!repo || !token) {
        status = 0;
        ok = false;
        error = "GH_TOKEN/GH_REPO not set";
      } else {
        ...
        const put = await ghPutContent(
          ...
          path,
          next,
          "chore(checks): scheduler execute noop"
        );
    ...
    results.push({
      key: "schedulerExecute",
      url: "gh:append data/scheduler/history.ndjson",
      ok,
      status,
      durationMs: Date.now() - t0,
      error,
  ...
  // debugWrite via GitHub contents
  {
    ...
    results.push({
      key: "debugWrite",
      url: "gh:append data/debug.ndjson",
      ok,
      status,
      durationMs: Date.now() - t0,
      error,
  ...
  // Github creds summary
  {
    ...
    results.push({
      ...
      status: ghOk ? 200 : 0,
      durationMs: 0,
      error: ghOk ? undefined : "GH_TOKEN/GH_REPO not set",
    });
  }
