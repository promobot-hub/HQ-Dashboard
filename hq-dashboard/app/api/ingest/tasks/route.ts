...
import { getRepo, ghPutContent, ghGetContent, readJson, verifySignature } from "../utils";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!tasks) return NextResponse.json({ ok:false, error:"missing tasks" }, { status: 400 });
    ...
    const snap = await ghPutContent(repo, snapshotPath, JSON.stringify({ ts: tsid, tasks }, null, 2), `chore(ingest): tasks snapshot @ ${tsid}`);
    // Merge or replace tasks.json (simple replace)
    const put = await ghPutContent(repo, tasksPath, JSON.stringify({ tasks }, null, 2), `chore(ingest): tasks update @ ${tsid}`);
