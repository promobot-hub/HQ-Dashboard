...
async function loadAgents(repo?: string) {
  ...
  if (r?.content && r.encoding==='base64') {
    try { const json = JSON.parse(Buffer.from(r.content, 'base64').toString('utf8')); return Array.isArray(json)? json : (json?.agents||[]); } catch { return []; }
  }
  return [] as any[];
}
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const ok = await ghPutContent(repo, PATH, JSON.stringify(agents, null, 2), `chore(agents): update ${id}`);
