...
export async function POST(req: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "GH_TOKEN and GH_REPO (target) required; optionally body.repo as source",
        },
        { status: 400 }
      );
    }
    ...
    skills[0].activity = (skills[0].activity || [])
      .slice(-50)
      .concat(activity.slice(-50));
    ...
    const put = await ghPutContent(
      ...
      `feat(skills): add activity snapshot from ${sourceRepo}`
    );
    return NextResponse.json(
      { ok: !!put.ok, repo, activity: activity.length },
      { status: 200 }
    );
  } catch (e: any) {
    return NextResponse.json(
