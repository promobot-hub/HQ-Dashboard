...
const rateMap: Map<string, { count: number; ts: number }> =
  (global as any).__rateMap || new Map();
...
export function middleware(req: NextRequest) {
  ...
  res.headers.set(
    ...
    "default-src 'self'; img-src 'self' data: https:; script-src 'self'; style-src 'self' 'unsafe-inline'; connect-src 'self' https://api.github.com https://raw.githubusercontent.com"
  );
  ...
  if (req.nextUrl.pathname.startsWith("/api/")) {
    ...
    const now = Date.now();
    const key = `${ip}:${
      req.nextUrl.pathname.startsWith("/api/ingest/") ? "ingest" : "api"
    }`;
