...
async function uploadFilesToGitHub() {
  ...
  for (const filePath of files) {
    if (filePath.includes('node_modules') || filePath.includes('.git')) continue; // skip node_modules and git dir
  ...
  // Erzeuge einen Tree mit allen Blobs ohne base_tree
  ...
  // Erstelle Commit mit dem Tree ohne Eltern (Erstcommit)
  ...
  const respCommit = await fetch(`https://api.github.com/repos/${owner}/${repo}/git/commits`, {
    ...
    body: JSON.stringify({ message: commitMessage, tree: baseTree.sha, parents: [] })
  });
